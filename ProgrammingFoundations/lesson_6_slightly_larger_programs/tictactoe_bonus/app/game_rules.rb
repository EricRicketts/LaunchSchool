...
module GameRules
  ...
  def detect_column_winner(board, selected_square, player)
    col_size = board.size
    column = decrement(selected_square) % col_size
    column_values = board[0..(col_size - 1)].map { |row| row[column] }
  ...
  def generate_and_check_either_diagonal(square_nums, board, row_size, player)
    decremented_square_numbers =
      square_nums.map { |square_number| decrement(square_number) }
