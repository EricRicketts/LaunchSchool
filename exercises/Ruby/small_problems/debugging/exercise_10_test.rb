...
=end
  ...
  # problem was original version did not conver the numbers to floats
  # so the resultign log often turned out to be 0.0

  number_of_documents = documents.length.to_f
  number_of_documents_with_term = documents.count { |d| tf(term, d) > 0 }.to_f

  number = number_of_documents / number_of_documents_with_term
  Math.log(number)
end
